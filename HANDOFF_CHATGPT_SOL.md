...
- CARLS DF renderer contract: every state refresh emits a complete four-row `set_carls_radio` payload. HPG officially permits `show_condition` inside Items, but this renderer materializes the selected row before refresh to prevent stale/blank layouts.
- HPG query contract: a global query is `{"global":"CARLS_DF_FREQUENCY"}`; `var` remains reserved for an LVAR array. No DF editor state uses LVARs.
- Release identity contract: the static tablet title must equal the first release heading in `CHANGELOG.en.md`; the validator enforces `0.997 94`.

### Runtime checks for release 94 CARLS DF
...
6. Confirm the tablet title reads `HEMS RANDOM AND EVERYWHERE MISSIONS 0.997 94`.
## Release 0.997 93
...
- Settings layout contract: FLIGHT ASSISTS and the standalone MEDICAL OPTIONS section set their local tab state to zero on every Settings open. Medical visit timing, treatment mode, CPR, LifeScore transport/cancellation and patient/crew ground-transfer controls use only the Medical tab visibility guard; the treatment-mode label uses default text color, not heading green. Pilot boarding renders directly under GROUND/HOIST with `tab10`, never as a loose `tab1` control below Flight Assist. A fixed bar separates MOST USED SETTINGS from FLIGHT ASSISTS.
- Renderer command contract: initialization/table commands remain executable, while every visual row is contained in `set_dispatch`. The validator now scans every primary and companion macro for top-level `image/title/link/text/buttonbar/describe_icon/slider/input` keys, preventing the HPG `NotFound` class globally rather than by page name. It also asserts the complete mission root shape and that persistent table mappings remain under root `data`, because syntactically valid JSON can still have an invalid hierarchy. The aircraft-profile page also normalizes its display name, opens the current custom table through a param-backed helper, disables mission-link buttons for factory profiles, and validates save/load/link-table parity.
- Static gate: PASS — 627 macro arrays, 7,022 executable conditions, 6,903 `require` leaves, 2,269 renderer conditions, 2,400 static and 3 guarded dynamic macro calls, 276 icon/image references, 45 CARLS layouts, 33 fixed-width BEFORE TAKE-OFF rows, 479 regression assertions, 8,806 top-level renderer-command checks and 11 companion assertions. `tools/test-crew-emergency.js` also passes role-mapping, targeted-impact, exact-10 critical, zero-score fatal, and fatal-survivor-boarding scenarios. Runtime testing remains mandatory.
...
- Preset contract: individual/group edits mutate the open table and set `preset_dirty=yes`; `flush mission preset` is the only writer and runs on switch/exit. Recompute is table-to-UI only. A partial category requires the same button twice to enable all; a fully disabled category enables all with one press; a fully enabled category disables all with one press. The confirmation message belongs only to the partial state.
