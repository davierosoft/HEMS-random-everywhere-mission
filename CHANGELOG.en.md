...
- Fixed the remaining CARLS DF first-key regression introduced after the last live-working revision. Transient editor state now stays in mission-local values, every keypad value remains a same-task `param`, and the working direct `if + comparator` capture path replaces the later `and/require` wrappers that consumed the first event. The release gate now executes a six-key state-transition test and proves that `121500` is captured without a missing digit.
