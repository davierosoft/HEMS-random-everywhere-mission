...
La PR43 e' stata chiusa. Eliminati tutti i 15 branch remoti superflui: rimangono main e CICERS/fix-local-time-query. In locale resta soltanto il branch operativo. Il fetch ora aggiorna tutti i riferimenti remoti. Prima di eliminare i due branch non antenati sono stati contabilizzati i loro contenuti: tutti i sette fallback sanitari di `1a44df6` e i relativi rami di successo sono presenti nei moduli correnti, il vecchio fallback object-literal e' assente e il changelog16 e' conservato. Le modifiche global/default/validator della PR43 sono gia' presenti; i rimanenti scostamenti sono versioni precedenti dei controlli. Nessuna fusione di sola storia e' stata eseguita.

I draft ora verificano il file canonico senza copiarlo. Allineate le istruzioni annidate e la checklist: le correzioni interne non incrementano la release. Il test include il rifiuto di contenuti diversi dal file canonico e la verifica che non venga creata una directory di copie draft. Il gate `check-canonical-artifact.js`, incluso nella suite, impedisce copie missione parallele e global.json nel sorgente; i test verificano sia il caso valido sia il rifiuto di una copia. Gli output di consegne esplicite e i metadati interni restano esclusi dalla scansione. Nessuna modifica semantica alla missione e nessuna nuova release.
